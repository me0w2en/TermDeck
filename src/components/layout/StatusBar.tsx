...
export default function StatusBar({ counts, totalCost, totalAllTimeCost, themeMode, onCycleTheme }: StatusBarProps) {
  return (
    <footer className="flex h-8 items-center justify-between rounded-xl bg-white/5 backdrop-blur-xl border border-white/10 px-3 text-[11px] text-white/50 gap-2 overflow-hidden">
      <div className="flex items-center gap-2 flex-shrink-0">
        {statusConfig.map(({ key, label, color }) => (
          <span key={key} className="flex items-center gap-1">
            <span className="inline-block h-1.5 w-1.5 rounded-full flex-shrink-0" style={{ backgroundColor: color }} aria-hidden="true" />
      ...
      <div className="flex items-center gap-2 min-w-0">
        ...
        {onCycleTheme && themeMode && (
          <button type="button" onClick={onCycleTheme} className="flex items-center gap-1 text-white/30 hover:text-white/60 transition-colors flex-shrink-0" title={`Theme: ${themeMode}`}>
